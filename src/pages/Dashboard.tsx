...
const Dashboard: React.FC<DashboardProps> = ({ stats, schedule = [], savedSchedules = [] }) => {
  const [selectedScheduleId, setSelectedScheduleId] = useState<string | null>(
    savedSchedules.length > 0 ? savedSchedules[savedSchedules.length - 1].id : null
  );
      ...
      {savedSchedules.length > 0 ? (
        <div className="mt-8">
          <div className="mb-4 flex items-center justify-between">
            <h2 className="text-xl font-semibold text-white">Production Chart</h2>
            <select 
          ...
          <ProductionChart schedule={selectedSchedule} />
        </div>
      ) : (
