...
export const AuthService = {
  ...
  getMyTools: async (): Promise<ToolResponse> => {
    try {
      ...
      if (axios.isAxiosError(error) && error.response) {
        throw new Error(error.response.data.message || 'Gagal mendapatkan daftar tools');
      }
      throw new Error('Terjadi kesalahan saat menghubungi server');
    }
  },
  ...
  async (error) => {
    if (axios.isAxiosError(error) && error.response) {
      ...
      if (error.response.status === 401) {
        // Cek apakah request URL adalah untuk login
        const isLoginRequest = error.config.url?.includes('/auth/login');
        
        // Jika bukan request login, baru redirect ke halaman login
        if (!isLoginRequest) {
          localStorage.removeItem("currentUser");
          localStorage.removeItem("userTools");
          window.location.href = "/login";
        }
        // Jika ini adalah request login, biarkan error ditangani oleh catch block di LoginPage
      }
    }
