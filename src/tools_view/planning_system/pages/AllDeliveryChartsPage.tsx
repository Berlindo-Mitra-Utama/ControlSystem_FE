...
const AllDeliveryChartsPage: React.FC = () => {
  ...
  // Build daily delivery chart data for selected month/year
  const generateMonthlyDeliveryData = (monthName: string) => {
    ...
    // Initialize all days
    const dayData: { [key: number]: { delivery: number } } = {};
    for (let day = 1; day <= daysInMonth; day++) {
      dayData[day] = { delivery: 0 };
    }
    ...
    monthSchedules.forEach((schedule) => {
      schedule.schedule.forEach((item) => {
        if (item.day >= 1 && item.day <= daysInMonth) {
          // Use item.delivery as main metric, fallback to item.akumulasiDelivery if present
          const val = item.delivery ?? item.akumulasiDelivery ?? 0;
          dayData[item.day].delivery += val;
        }
      });
    });
    ...
    return Object.keys(dayData)
      .map((dayStr) => {
        ...
        return {
          day: day.toString(),
          akumulasiDelivery: dayData[day].delivery,
          originalDay: day,
        };
      })
      ...
      <div className="w-full max-w-7xl mx-auto px-2 sm:px-6 lg:px-8 py-4 sm:py-8">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-4 mb-4">
            ...
            <h1 className={`text-3xl font-bold ${uiColors.text.accent}`}>
              Semua Chart Akumulasi Delivery (Per Hari)
            </h1>
          </div>
          <p className={`text-lg ${uiColors.text.secondary}`}>
            Per hari untuk bulan dan tahun yang dipilih. Pilih part untuk
            memfilter data.
          </p>
        </div>
        ...
        <div className="space-y-8">
          {filteredMonths.map((month) => {
            const chartData = generateMonthlyDeliveryData(month);
            ...
            return (
              ...
              >
                ...
                <div className="mb-6">
                  ...
                  <p className={`text-lg ${uiColors.text.secondary}`}>
                    {selectedPart
                      ? `Akumulasi Delivery - ${selectedPart} (Per Hari)`
                      : "Akumulasi Delivery (Per Hari)"}
                  </p>
                </div>
                  ...
                  <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-12 border border-gray-200 dark:border-gray-700">
                    <div className="text-center">
                      ...
                      <p className={`text-sm ${uiColors.text.tertiary}`}>
                        {selectedPart
                          ? `Belum ada data delivery untuk ${selectedPart} di bulan ${month} ${selectedYear}`
                          : `Belum ada data delivery di bulan ${month} ${selectedYear}`}
                      </p>
                    </div>
                  </div>
                )}
...
export default AllDeliveryChartsPage;
